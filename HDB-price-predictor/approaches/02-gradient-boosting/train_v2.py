"""
HDB Resale Price Prediction — Training Script v2
==================================================
Uses features_v2.py pipeline with Phase 1 improvements and
K-Fold CV with OOF stacking for the ensemble.

Base models: RF, XGBoost, LightGBM, CatBoost
Meta-learner: RidgeCV trained on out-of-fold predictions

Run from the project root: python approaches/02-gradient-boosting/train_v2.py
"""
import sys, os
...
def fit_model(model, name, X_train, y_train, X_val, y_val,
    ...
    """Fit a model with appropriate arguments per type."""
    if name == "CatBoost":
...
def predict_model(model, name, X_ohe, X_raw=None):
    """Predict with the correct input format per model type."""
    if name == "CatBoost":
...
def main():
    ...
    for fold_i, (train_idx, val_idx) in enumerate(kf.split(X_trainval)):
        ...
        # Encode this fold (target encoding fitted on fold-train only)
    ...
    # ── Stacked Ensemble (constrained optimal weights) ──────────
    print(f"\n\n{'=' * 75}")
    print("  STACKED ENSEMBLE (Constrained Optimal Weights)")
    print(f"{'=' * 75}")

    # Find weights that minimize MSE on OOF predictions
    # Constraints: weights >= 0, sum to 1 (convex combination)
    def mse_objective(w):
    ...
    oof_ens_result = evaluate("Stacked Ensemble (OOF)", y_trainval, ens_oof)
    test_ens_result = evaluate("Stacked Ensemble (test)", y_test, ens_test)

    # ── Summary table ────────────────────────────────────────────
    for label, idx in [("OOF (TRAINVAL)", 1), ("TEST", 2)]:
        print(f"\n\n{'=' * 75}")
        ...
        r = oof_ens_result if idx == 1 else test_ens_result
        print(
            f"  {'Stacked Ensemble':<30} "
            f"${r['rmse']:>11,.0f} "
            f"${r['mae']:>11,.0f} "
            f"{r['mape']:>9.2f}% "
            f"{r['r2']:>7.4f}"
